...
async function testDataSize(data: unknown): Promise<boolean> {
...
export async function fixOutstandingPlacementsMigration(): Promise<{
  success: boolean;
  message: string;
  data?: unknown | null;
}> {
  const results: { success: boolean; message: string; data?: unknown | null } = { success: false, message: '', data: null };
  ...
  try {
    ...
    if (error instanceof Error) {
      ...
      results.message = `Error: ${String(error)}`;
    }
    results.success = false;
  }
